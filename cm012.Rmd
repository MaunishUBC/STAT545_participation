...
```
sample function draws randomly, set.seed ensures that the results of the randomization are going to be same every time. 
...
draw
levels(draw)
nlevels(draw) # Tells me how many factors are there
fct_count(draw)
library(forcats)
```

Levels are the possible catagories that each entry can contain. 
What's the differnce between factor and as.factor
...
ggplot(gap_small, aes(x = continent)) + 
  geom_bar() + 
...
```{r}
gap_small %>% str()
droplevels(gap_small) %>% str()

gap_small %>% 
  mutate(continent =  fct_drop(continent), country = fct_drop(country)) %>% str()
...
```{r}
gap_asia_2007 %>% mutate(country = fct_reorder(country, lifeExp)) %>% 
...
fct_reorder2 uses the maximum value of y at the maximum value of x for a 2D plot.
...
```
Because it is 2D fct_reorder2 needs to be given two inputs in this country, year and lifeExp
