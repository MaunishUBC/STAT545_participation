...
```{r}
gapminder %>% mutate(gdp = gdpPercap * pop, gdpBill = round(gdp/10^9,2))
```
...
```{r}
gapminder %>% transmute(gdp = gdpPercap * pop, gdpBill = round(gdp/10^9,2))
```
...
```{r}
gapminder %>% mutate(lifeExp = if_else(country == "Canada" & year == 1952, 70, lifeExp)) %>% filter(country =="Canada")
...
gapminder %>% 
  group_by(continent,country) %>% 
  summarize(md_gdp = median(gdpPercap)) %>% 
  summarize(min = min(md_gdp))
```
...
gapminder %>% 
  ...
  summarize(md_gdpPercap = median(gdpPercap))
...
gapminder %>%
  group_by(country) %>% 
  mutate(gdp_rel = gdpPercap/(gdpPercap[country == "Canada"]))
