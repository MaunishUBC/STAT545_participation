...
as.character(100)
as.numeric("100")*2
as.numeric("hello") # This wouldn't work as R doesn't know what to do. 
class(as.data.frame(letters))
...
course[-1]
```
Some programming languages have a base 0 programming language but R starts from 1. 
...
course
```
Can change bits and pieces of the vector using the assignment operator
...
is.vector(6)
c("hi", "you")
d <- "Hi, how are you? Today is the 9th."
length(d)
d[1]
d[2]
paste("hi", "you")
length(paste("hi", "you"))
```
R doesn't distinguish the vector and it's elements
...
(n <- length(a))
```
By putting parathesis along the assigned vector, it will also print it out.
...
a^2
```
# In base python to do this we'd have to use a for loop
...
a * 1:2
```
Is it going to give an error? Drop the rest? Spit out NAs? 
So, it actually recycles the variable and if the longer object is not a multiple of the shorter object, then it gives a warning. 

Typically we don't want mismatch in vector length. 
...
Let's return vectors that are positive and lesser than 5
```{r}
a < 5
a > 0 & a < 5

# Use functions "all" or "any" to reduce a vector to non-1 length.
all(a > 0 & a < 5)

```
So, whats the differnce between the & and && 
In if statements we want a single output to compare hence && and || are not vectorized. Logically they do the same thing, but the single ones (& and |) are vectorized. 
...
```
We can also assign names to the elements of the vectors
...
```
Think of list as train which can have a lot in it. 
...
my_list[[3]]
```
The double bracket sheds the list nature of the list and gives us just the content
...
as.list(small_df)
# Matrix
diag(5) # Allows the generation of an identity matrix

```

Note that there's a difference between a list of one object, and that object itself! This is different from vectors. 
Also each column needs to have the same datatype: which is derived from it's nature of being a list. 


Dataframe is also a list of vectors but each of the vectors MUST have the same length.
