...
| facetting             |  none |
Note: facetting is for side by side plotting
...
ggplot(gapminder, aes(x=lifeExp, y=gdpPercap)) + 
  geom_point()
```
We need the aes function. Not having it is a common source of error
So, geom_point is a layer and where you write aes will determine how I do it 
...
```{r}
ggplot(gapminder, aes(x=lifeExp)) +
  geom_histogram(bins= (max(gapminder$lifeExp)-min(gapminder$lifeExp)))
```
What is bins? 
Dividing the entire range of the data into that many bins


4. Instead of a histogram, let's create a kernel density plot.
This gives a smooth curve: Setups a kernel function
```{r}
ggplot(gapminder, aes(x=lifeExp)) +
...
|-----------------------|---------------|
| __data__              | `gapminder` |
| __aesthetic mapping__ |  `x is catagorical and y is numerical`|
| __geometric object__  |  `box plot` |
| scale                 | `log-y` |
...
```
Alpha is the inverse of the opacity of the points
...
```
To look at outliers and more importantly to look at the number of points in the dataset
